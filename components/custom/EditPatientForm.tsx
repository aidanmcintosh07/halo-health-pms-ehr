...
function EditPatientForm({ patientData }: EditPatientFormProps) {
	...
	const onSubmit = async (values: z.infer<typeof formSchema>) => {
		try {
			setLoading(true);
			await axios.patch(`/api/medical-record/${params.userId}`, values);
			router.refresh();
			toast({
				description: "Form editted successfully",
				duration: 3000,
			});
			router.push(`/medical-record/${params.userId}`);
		} catch (error) {
	...
	const onDelete = async () => {
		try {
			setLoading(true);
			await axios.delete(`/api/medical-record/${params.userId}`);
			router.refresh();
			toast({
